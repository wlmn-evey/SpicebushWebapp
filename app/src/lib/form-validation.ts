...
export const validators = {
  required: (value: string): string | null => (value?.trim() ? null : 'This field is required'),
  ...
  matches:
    (fieldName: string, getValue: () => string) =>
    (value: string): string | null =>
...
export function validateForm(
  ...
  for (const [field, rules] of Object.entries(schema)) {
    const value =
      formData instanceof FormData ? formData.get(field)?.toString() || '' : formData[field] || '';
