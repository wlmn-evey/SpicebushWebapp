...
node -e "try { require('astro'); console.log('✓ Node can resolve modules correctly'); } catch(e) { console.error('✗ Node cannot resolve modules:', e.message); if (e.message.includes('rollup')) { console.log('⚠ Ignoring optional rollup dependency issue'); } else { process.exit(1); } }"
...
# No longer needed - npm will detect correct architecture automatically
