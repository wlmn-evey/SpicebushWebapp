...
async function testSpecificFunctionality(browser) {
  ...
  try {
    ...
    let formInputs = [];
    if (hasForm) {
      formInputs = await tourPage.$$eval('form input, form textarea, form select', 
        els => els.map(el => ({ 
...
async function testMobileView(browser) {
  ...
  try {
    ...
    // Check for mobile menu - use simpler selectors
    const mobileMenu = await page.$('[aria-label*="menu"], .mobile-menu, .hamburger, button');
